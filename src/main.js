...
if (!supabaseUrl || !supabaseKey) {
    ...
    showGlobalError('Ошибка: Не настроены ключи Supabase');
}
...
async function testConnection() {
    try {
        ...
        if (error) {
            ...
            showGlobalError('Ошибка подключения к базе данных');
            return false;
        }
        ...
        showGlobalError('Ошибка подключения');
        return false;
    }
}
// Отображение фильмов
...
// 1. Загрузка всех фильмов
async function loadAllFilms() {
    try {
        console.log('Loading films from Supabase...');
        ...
        if (error) {
            ...
            showError('filmsGrid', 'Ошибка загрузки фильмов из базы');
            return;
        }

        console.log('Films loaded successfully:', films);
        ...
        showError('filmsGrid', 'Неожиданная ошибка');
    }
}
...
window.testConnection = async function() {
    try {
        ...
        alert('Подключение к Supabase успешно! Загружено фильмов: ' + data.length);
